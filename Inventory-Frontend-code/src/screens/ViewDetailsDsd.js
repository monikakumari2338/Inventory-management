...
const ViewDetailsDsd = ({route}) => {
  ...
  useEffect(() => {
    axios
      ...
      .get(`http://172.20.10.9:8083/dsd/findby/invoicenumber/${invoiceNumber}`)
      .then(response => {
        ...
        <View style={{flex: 1}}>
          <TouchableWithoutFeedback onPress={closeMenu}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              ...
              <Text
                onPress={() => navigateToScreen('ViewDSD')}
                style={{
                  top: 3,
                  fontSize: 30,
                  marginLeft: 2,
          ...
          <ScrollView contentContainerStyle={styles.container}>
            {invoiceData &&
              invoiceData.map((res, index) => (
                <View key={res.itemNumber} style={styles.productCard}>
                  <Image
